...
def print_solution(data, manager, routing, solution, took_time, filename):
    ...
    line = (f'Total distance of all routes: {total_distance}m\n'
            f'Total load of all routes: {total_load}\n'
            f'Total time {took_time} sec')
...
def main():
    ...
    # OBJECTIVE_TABU_SEARCH Uses tabu search on the objective value of solution to escape local minima (Note: not working)
    for search_strategy_name, search_strategy in [
        ...
        for time_limit in range(1, 7):
            ...
            search_parameters = pywrapcp.DefaultRoutingSearchParameters()
            search_parameters.local_search_metaheuristic = (search_strategy)
            ...
            if solution:
                print_solution(data, manager, routing, solution, end - start, f'./results/{search_strategy_name}-timelimit{time_limit:02d}-sec.txt')
